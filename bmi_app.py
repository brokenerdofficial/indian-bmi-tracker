...
<style>
    ...
    * { font-family: 'Inter', sans-serif !important; }
    ...
    /* --- MOBILE 50% WIDTH FIX --- */
    /* This forces columns to stay side-by-side (50% width) on mobile devices */
...
# 1. Gender (Full Width for easy selection)
gender = st.selectbox("Gender", ["Male", "Female"])

# 2. Age & Weight (Side-by-Side 50% on Mobile)
c1, c2 = st.columns(2)
with c1: age = st.number_input("Age", 10, 100, 25)
with c2: weight = st.number_input("Weight (kg)", 1.0, 300.0, 72.0)

# 3. Height Fields (Side-by-Side 50% on Mobile)
c3, c4 = st.columns(2)
...
if calc:
    ...
    # 1. BMI & TDEE (Side by Side - 50% split on mobile)
    ...
    <div class="pro-card">
        ...
        <div class="metric-sub">kcal / day ({plan['Goal']})</div>
    </div>
    """, unsafe_allow_html=True)

    # 3. Diet Plan (2x2 Grid - 50% split on mobile)
